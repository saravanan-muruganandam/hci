...
/*
	...
	xmlhttp.open("POST","http://localhost/canvas/circle_post.php",false);
	xmlhttp.setRequestHeader("Content-type","application/x-www-form-urlencoded");
	//xmlhttp.send("{\"id\":0,\"name\":\"\",\"x\":"+x+",\"y\":"+y+",\"marked_id\":0}");
	//alert("x="+x+"&y="+y);
...
function mapInfoGet(map_id){
	...
	$.ajax({
		...
		success: function(data, textStatus){
			...
			img_url = "http://"+server_host+data.url;
			//alert(cid_temp);
			state = true;
		},
		complete: function(XMLHttpRequest, textStatus){
...
function mapImgGet(map_url){
	//don't know how to get img with ajax
}
...
function lineGet(map_id){
	...
	$.ajax({
		...
		success: function(data, textStatus){
			...
			for(var i=0;i < data.length;i++){//load the circles from data
				//circles.push(new Circle(x,y));
				var l_temp = new localCircle(data[i].id,data[i].start_c,data[i].end_c);				
				lines.push(l_temp);				
			}
			state = true;
			alert("lines get ok");
