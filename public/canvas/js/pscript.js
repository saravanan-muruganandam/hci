...
function markApply(){
	...
	$.ajax({
		...
		type: "POST",
		url: "http://"+server_host+"/maps/1/nodes.json",
		data:{'type':3,'node_id':circles[hoveredCircle].id},
		beforeSend: function(XMLHttpRequest){
			//ShowLoading();
		},
		success: function(data, textStatus){
...
function markDel(){
	...
	$.ajax({
		...
		type: "POST",
		url: "http://"+server_host+"/maps/1/nodes.json",
		data:{'type':4,'node_id':circles[hoveredCircle].id},
		beforeSend: function(XMLHttpRequest){
			//ShowLoading();
		},
		success: function(data, textStatus){
			//var result = eval('('+data+')');
			//alert(result.id);
			circles[hoveredCircle].mark_id = 0;
			state = true;

			document.getElementById("mark_id").innerHTML = "";
			document.getElementById("mark_id").innerHTML = "markId deleted";
		},
		complete: function(XMLHttpRequest, textStatus){
			//HideLoading();
		},
		error: function(){
			alert("mark del failed. Please try again later")
		}
	});
...
function change(){
	document.getElementById('button_test').innerHTML = "";
	document.getElementById('button_test').innerHTML = "rock";
	//$('#button_test').button("refresh");
}
...
$(function(){
	...
	$("#mark_apply").button();
	$("#mark_apply").click(function(){
		if(circles[hoveredCircle].mark_id){//there is a mark_id
			markDel();
			document.getElementById("mark_apply").innerHTML = "markApply";			
		}else{
			markApply();
			document.getElementById("mark_apply").innerHTML = "markDel";
		}
	});
	...
	$("#cir_dia_btn").click(function(){//change the circle name		
		...
		retry_time = 0;
		do{
			c_name_mod_flag = circleAjaxUpdate(circles[hoveredCircle].id,circles[hoveredCircle].x,circles[hoveredCircle].y,document.getElementById("circle_name").value);
			//alert(document.getElementById("circle_name").value);
			retry_time++;
	...
	document.getElementById("canvas_div").style += window.innerWidth-250;
		...
		if(move_flag){
			...
			if(hoveredCircle != undefined){
				...
				if(!circles[hoveredCircle].mark_id){
					...
					document.getElementById("mark_apply").innerHTML = "";
					document.getElementById("mark_apply").innerHTML = "markApply";
				}else{
					document.getElementById("mark_id").innerHTML = circles[hoveredCircle].mark_id;
					document.getElementById("mark_apply").innerHTML = "";
